...
public class TimeManager : MonoBehaviour {
	...
	void Update () {//this method called every frame to sync movement positions of remote players
		if (!running) return;		
		if (lastRequestTime > period) {
			lastRequestTime = 0;
			timeBeforeSync = Time.time;
			NetworkManager.Instance.TimeSyncRequest();//get server current time
		}
		else {
